<div id="footerContainer" class="   left-0 w-screen lg:border-t  {{ $isDarkTheme ? 'border-t-green lg:border-t-white' : 'border-t-green lg:bg-white' }} {{ isset($isAboutNav)  ? 'fixed bottom-0 transform translate-y-[200%] overflow-hidden z-[64]' : 'fixed bottom-0 z-[60]'  }}">
    <ul  id="footerList" class=" lg:py-6 w-full lg:px-8  text-white lg:flex items-center justify-between ">
        ...
        @if($isDarkTheme)
        <li id="reservationLink" class="text-xl text-right  px-8 py-4 md:py-6 lg:py-0 lg:px-0 lg:border-none border-t border-t-white  opacity-0 transform translate-y-full animate-pop-up" :class="[ menuOpen ? 'text-green delay-500':'text-white delay-100' ]" style="animation-delay: 0.4s;">
        ...
        @else
        <li class="text-xl bg-white lg:pb-0  text-right px-8 py-4 md:py-6 lg:py-0 lg:px-0 lg:border-none border-t border-t-green  opacity-0 transform translate-y-full animate-pop-up text-green" :class="[ menuOpen ? 'border-green delay-500':'' ]" style="animation-delay: 0.4s;">
            <a href="https://reservations.tablebooker.com/?restaurantId=51380481&obmPrimaryColor=%23063e33&obmFontColor=%23f6eee3&lang={{ Config::get('app.locale') }}" target="_blank" class="pb-1" data-hover >{{ __('home.reservation')}}</a>
        </li>
        @endif
